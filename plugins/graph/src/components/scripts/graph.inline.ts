...
(function () {
  ...
  function initGraph() {
    ...
    function addToVisited(slug) {
      ...
      visited.add(normalizeGraphSlug(slug));
    ...
    function normalizeGraphSlug(slug) {
      ...
      normalized = simplifySlug(normalized);
      if (normalized === "") normalized = "index";
      return normalized;
    }

    function readableFallbackTitle(slug) {
      var normalized = normalizeGraphSlug(slug);
      if (normalized.startsWith("tags/")) return "#" + normalized.substring(5);
      var last = normalized.split("/").filter(Boolean).pop() || normalized;
    ...
    async function renderGraph(graph, fullSlug, renderGeneration) {
      ...
      data.forEach(function (details, source) {
        ...
        for (var i = 0; i < outgoing.length; i++) {
          var dest = normalizeGraphSlug(outgoing[i]);
      ...
      neighbourhood.forEach(function (url) {
        ...
        var text = isTag ? "#" + url.substring(5) : data.get(url)?.title || readableFallbackTitle(url);
        ...
        .force(
          ...
          d3
            ...
            .radius(function (d) {
              ...
              return 6 + Math.sqrt(numLinks) * 1.6;
            })
            .iterations(3),
        );
      ...
      for (var i = 0; i < nodes.length; i++) {
        ...
        label.alpha = nodes.length <= 4 ? 0.65 : 0;
